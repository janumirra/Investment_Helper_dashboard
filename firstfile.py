...
graph_drop = st.sidebar.selectbox(
    'Which Graph_detail would you like to view?',
    ('GDP', 'CPI', 'CLI'))

Year_chosen = st.sidebar.slider(
...
misc_df= pd.read_excel("Bhumio Asnmnt.xlsx",'misc')
...
with left_column:
  ...
  fig_p=px.pie(
    data_frame=misc_df,
    values= 'Debt to GDP percentage',
    names= 'Country',
    color='Country',
    color_discrete_sequence=['orange','blue','green','cyan','yellow'],
    hover_name= 'Revised Debt in Billion $',
    labels={'country': 'The Country'},
    title='Countries by Debt levels and CAGR in %',
    template='plotly_white',
    
        )
  fig_p.update_traces(textposition='outside',textinfo='percent+label',
                        marker=dict(line=dict(color='#000000', width=4)),
                        pull=[0,0,0.2,0,0], opacity=0.8, rotation=180)
  fig_p.update_layout()
  st.plotly_chart(fig_p)  
    
with right_column:
  df=misc_df.drop(columns=['Debt to GDP percentage','Revised Debt in Billion $'])
  fig_b = px.bar(
    data_frame=df,
    x="Country",
    y=['Ease of doing business index','Corruption level index'],
    barmode='group',
    title="Which country is easy to do business?",
    template='plotly_white'
        )
