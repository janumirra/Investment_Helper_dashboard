...
st.sidebar.header(":chart_with_upwards_trend: Investment Helper App")
...
if graph_drop=='GDP':
    ...
    fig = px.line(dff, x="Time", y=df.columns,
              ...
              title='Which country has rising economy?')
    fig.update_xaxes(
      dtick="M1",
      tickformat="%b\n%Y",
    ...
    fig = px.line(dff, x="Time", y=df.columns,
              hover_data={"Time": "|%B , %Y"},# only month and year
              title='Which country has increased confidence in near-future business performance?')
    fig.update_xaxes(
      dtick="M1",
      tickformat="%b\n%Y",
      ticklabelmode="period")
    ...
    fig = px.line(dff, x="Time", y=df.columns,
              hover_data={"Time": "|%B , %Y"},# only month and year
              title='Which country has increasing inflation rate?')
    fig.update_xaxes(
            dtick="M1",
            tickformat="%b\n%Y",
            ticklabelmode="period")
